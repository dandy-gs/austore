...
if (isset($_GET['id'])) {
    if ($_GET['id'] != "") {
        ...
        $query = mysqli_query($conn, "DELETE FROM baju_rajut WHERE id='$id'");
        if ($query) {
            header("location:../../index.php?hal=baju&pesan=hapus");
        } else {
            header("location:../../index.php?hal=baju&pesan=gagalhapus");
        }
    } else {
