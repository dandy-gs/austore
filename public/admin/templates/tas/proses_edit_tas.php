...
if (isset($_POST['id'])) {
    ...
    if ($foto_size > 2097152) {
        ...
        header("location:../../index.php?hal=edit_tas&pesan=size");
    } else {
        ...
        if ($foto_nama != "") {
            ...
            if (in_array($ekstensi, $ekstensi_izin) === true) {
                ...
                move_uploaded_file($file_tmp, './templates/tas/file/' . $foto_nama_new);

                // Query untuk memasukan data kedalam table SISWA
                $query = mysqli_query($conn, "UPDATE tas_rajut SET nama='$nama',jenis='$jenis',harga='$harga',foto='$foto_nama_new',lama_buat='$lama_buat' WHERE id='$id'");

                // Mengecek apakah data gagal diinput atau tidak
                if ($query) {
                    header("location:../../index.php?hal=edit_tas&pesan=berhasil");
                } else {
                    header("location:../../index.php?hal=edit_tas&pesan=gagal");
                }
            } else {
                // Jika ekstensinya tidak sesuai dengan apa yg kita tetapkan maka error
                header("location:../../index.php?hal=edit_tas&pesan=ekstensi");
            }
        } else {

            // Apabila tidak ada file yang diupload maka akan menjalankan code dibawah ini
            $query = mysqli_query($conn, "UPDATE tas_rajut SET nama='$nama',jenis='$jenis',harga='$harga',lama_buat='$lama_buat' WHERE id='$id'");
