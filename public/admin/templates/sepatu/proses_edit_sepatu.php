...
if (isset($_POST['id'])) {
    ...
    if ($foto_size > 2097152) {
        ...
        header("location:../../index.php?hal2=edit_sepatu&pesan=size");
    } else {
        ...
        if ($foto_nama != "") {
            ...
            if (in_array($ekstensi, $ekstensi_izin) === true) {
                ...
                $get_foto = "SELECT foto FROM sepatu WHERE id='$id'";
                $data_foto = mysqli_query($conn, $get_foto);
                // Mengubah data yang diambil menjadi Array
                $foto_lama = mysqli_fetch_array($data_foto);

                // Menghapus Foto lama didalam folder FOTO
                unlink("./templates/sepatu/file/" . $foto_lama['foto']);

                // Memindahkan File kedalam Folder "FOTO"
                move_uploaded_file($file_tmp, './templates/sepatu/file/' . $foto_nama_new);

                // Query untuk memasukan data kedalam table SISWA
                $query = mysqli_query($conn, "UPDATE sepatu SET nama='$nama',jenis='$jenis',harga='$harga',foto='$foto_nama_new',lama_buat='$lama_buat' WHERE id='$id'");

                // Mengecek apakah data gagal diinput atau tidak
                if ($query) {
                    header("location:../../index.php?hal2=edit_sepatu&pesan=berhasil");
                } else {
                    header("location:../../index.php?hal2=edit_sepatu&pesan=gagal");
                }
            } else {
                // Jika ekstensinya tidak sesuai dengan apa yg kita tetapkan maka error
                header("location:../../index.php?hal2=edit_sepatu&pesan=ekstensi");
            }
        } else {

            // Apabila tidak ada file yang diupload maka akan menjalankan code dibawah ini
            $query = mysqli_query($conn, "UPDATE sepatu SET nama='$nama',jenis='$jenis',harga='$harga',lama_buat='$lama_buat' WHERE id='$id'");
